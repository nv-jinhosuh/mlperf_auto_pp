...
class ResNetBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if groups != 1 or base_width != 64:
            raise ValueError('ResNetBasicBlock only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in ResNetBasicBlock")
...
class DeepLabV3Plus_ResNet50(nn.Module):
    ...
    def __init__(self, checkpoint=None, target_device='cpu', num_classes=19, output_stride=16, 
        ...
        if checkpoint is not None:
            ...
            self.model.load_state_dict(loaded_model_state["model_state"])
        self.model.to(self.device)
        print(f"DeepLabV3Plus_ResNet50 model was loaded in device:{target_device}")
